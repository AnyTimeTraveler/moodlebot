...
public class CourseDetailsCallback implements Callback {
    ...
    public void execute(AbsSender bot, CallbackQuery callbackQuery) {
        ...
        try {
            ...
            for (CourseContent courseDetail : courseDetails) {
                if (courseDetail.getModules().size() > 2) {
                    menu.addButton("Subsection: " + courseDetail.getName(), new CouseModulesCallback(this, courseDetail));
                } else {
                    generateCourseModuleCallbacks(callbackQuery, userData, menu, courseDetail);
                }
            }
    ...
    void generateCourseModuleCallbacks(CallbackQuery callbackQuery, UserData userData, VerticalMenu menu, CourseContent courseDetail) {
